...
- 已完成的质量阶段使用 `--curriculum floor_mass_v2`：保持上述 reset 与 floor 摩擦分布，只逐步加入每个 body 耦合的质量/惯量 ±2% 和 ±5%；
- 当前下一阶段使用 `--curriculum floor_mass_gain_v3`：固定质量/惯量 ±5%，只逐步加入每个 actuator 独立的 `kp` ±2% 和 ±5%；
- `--physics-profile cg20`：训练与 reference 验证使用相同求解器。

旧 `reset_v1/reset_v2/nominal_reset_v3` 和旧 recipe 全部保留用于复现实验，但不再作为当前独立关节鲁棒性实验的推荐入口。`friction_v1` 也保留原来的全局 geom 摩擦语义。质量阶段见 [`3d_mass_inertia_curriculum_zh.md`](3d_mass_inertia_curriculum_zh.md)，当前执行器阶段见 [`3d_actuator_gain_curriculum_zh.md`](3d_actuator_gain_curriculum_zh.md)；root 扰动仍不同时加入。
