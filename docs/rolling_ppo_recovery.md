...
**已收到旧运行诊断包。优先执行 [2026-09-11 诊断与更新后的命令](rolling_ppo_diagnosis_20260911.md)。** 新一键入口是 `bash scripts/run_rolling_ppo_recovery.sh critic`；actor 阶段采用 KL 自适应学习率。下文保留之前的手动方案及对照评估方法，其中训练参数以新入口为准。
