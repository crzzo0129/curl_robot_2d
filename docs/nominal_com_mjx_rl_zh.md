...
立即产生极端关节目标。每个 episode 的前 0.25 s 复用 CEM 控制器的
smoothstep ramp，令动作幅值按 \(u^2(3-2u)\) 从 0 增长到 1；四个 action
scale 保持 `(0.8, 1.2, 0.8, 1.2)`，不添加动作变化率硬限制。
...
逐步进度为 \(\Delta\Phi_t=\Phi_t-\Phi_{t-1}\)。因此旋转和平移即使发生在
相邻控制周期，只要累计结果一致，仍能获得完整滚动奖励；原地旋转或纯滑移
...
与 CPU CEM 和最终 release evaluator 一致，三个档位在加载 XML 后都会清零
`root_x/root_z/root_pitch` 的数值阻尼。四个受控关节及执行器的阻尼不变。
