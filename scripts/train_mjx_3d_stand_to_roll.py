...
def _train_ppo(args, stage_out):
    ...
    def selection_key(row):
        ...
        if args.limit_torque:
            quality = row.get("eval/episode_reward_handoff_bonus", 0.0) / max(capture, 1e-12)
            return (capture, row.get("eval/episode_insurance_success", 0.0), quality,
    ...
    best_report = {
        "selection_order": (["capture_rate", "insurance_success_rate", "higher_mean_capture_handoff_bonus",
                             "shorter_mean_capture_time"] if args.limit_torque
...
def _capture_task(args):
    ...
    return replace(stand_to_roll_curriculum_config(args.stage),
                   ...
                   reward_torque_excess=0.0,
                   reward_lateral_before_capture=0.0,
                   reward_handoff_y=0.0,
