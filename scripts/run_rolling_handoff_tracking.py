"""Cloud continuation for heading-frame tracking from measured handoff and mature rolling states."""
import argparse
...
def main():
    ...
    parser.add_argument('--source', type=Path, default=Path('results/rolling_dr025_lateral050_20260912_213612'))
    parser.add_argument('--step', type=int, default=696320)
    parser.add_argument('--handoff-bank', type=Path, required=True)
    ...
    if not args.handoff_bank.is_file():
        parser.error('Missing --handoff-bank; run collect_rolling_handoff_bank first')
    ...
    out = (args.out or Path('results/rolling_heading_handoff_' + datetime.now().strftime('%Y%m%d_%H%M%S'))).resolve()
    common = [sys.executable, '-u', '-m', 'scripts.train_mjx_3d_roll_student_dr_ppo', str(student),
        ...
        '--rolling-hip-knee-kp-scale', '1.0',
        '--forward-velocity-frame', 'heading', '--handoff-bank', str(args.handoff_bank.resolve()),
        '--handoff-fraction', '0.5', '--handoff-speed-slew-m-s2', '0.15',
        '--handoff-initial-speed-max-m-s', '1.05', '--terminate-lateral-drift-m', str(args.lateral_limit_m),
    ...
    for stage in stages:
        ...
        if stage == 'critic':
            ...
            options = ['--restore-ppo', str(critic), '--steps', str(args.actor_steps), '--num-evals', '17',
                       ...
                       '--desired-kl', '0.01', '--min-learning-rate', '0.0000001', '--max-learning-rate', '0.000003']
    ...
    print('Nominal monitor: heading-frame speed, all P=5, D=0.1, lateral=%.2fm.' % args.lateral_limit_m)
    ...
    # Runs on the training host before expensive pool compilation or updates.
    subprocess.run([sys.executable,'-m','unittest','discover','-s','tests',
                    '-p','test_rolling_handoff_tracking.py','-v'],check=True)
    ...
    with manifest.open('x', encoding='utf-8') as handle:
        json.dump(dict(source=str(source), source_step=args.step, dr_strength=args.dr_strength,
                       lateral_limit_m=args.lateral_limit_m, handoff_bank=str(args.handoff_bank.resolve()), commands=commands), handle, indent=2)
