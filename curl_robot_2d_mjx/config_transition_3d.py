...
TRANSITION_SINGLE_OBSERVATION_SIZE_3D = ROLLING_DEPLOY_SINGLE_OBSERVATION_SIZE_3D
TRANSITION_OBSERVATION_HISTORY_3D = ROLLING_DEPLOY_OBSERVATION_HISTORY_3D
TRANSITION_ACTOR_OBSERVATION_SIZE_3D = (
    TRANSITION_SINGLE_OBSERVATION_SIZE_3D * TRANSITION_OBSERVATION_HISTORY_3D
)
...
class Transition3DConfig:
    ...
    Twelve actions are centered on the Walking startup pose in ALL modes.
    Curriculum changes initial states, never plays a braking/deployment motion.
    """
    ...
    # train_ppo_deploy.DeployEnv: NOMINAL_H + 0.0005, identity quaternion,
    # DEFAULT_POSE and zero qvel. No settling or invented intermediate pose.
    walking_start_height_offset_m: float = 0.0005
    # Current ROS manager 520 Hz / repeat_action 10 = nominal 52 Hz inference.
    physics_timestep: float = 1.0 / (HARDWARE_POLICY_FREQUENCY_HZ_3D * 20)
    ...
    # Fixed per-joint symmetric scale, then joint-limit clipping: same as C++.
    action_range_fraction: float = 1.0

    brake_timeout_s: float = 2.5
    deploy_timeout_s: float = 3.0
    ...
    ready_root_height_min_m: float = 0.1280029248
    ready_root_height_max_m: float = 0.1930029248
    ...
    reset_compact_fraction_range: tuple[float, float] = (0.0, 0.0)
    reset_joint_noise_rad: float = 0.0
    reset_joint_velocity_rad_s: float = 0.0
    reset_linear_speed_m_s: float = 0.0
    reset_angular_speed_rad_s: float = 0.0
    reset_tilt_rad: float = 0.0
    reset_roll_phase_range_rad: tuple[float, float] = (0.0, 0.0)
...
def ready_to_walk_reasons_3d(
    ...
    reasons: list[str] = []
    if not sample.finite or not all(math.isfinite(value) for value in (
        sample.linear_speed_m_s, sample.angular_speed_rad_s,
        sample.upright_tilt_rad, sample.joint_error_rad,
        sample.root_height_m, sample.foot_contacts,
    )):
...
def transition_curriculum_config_3d(
    ...
    if stage == "walking_start":
        result = replace(
            base, curriculum_stage=stage,
            reset_start_mode=int(TransitionMode3D.STABILIZE),
            reset_compact_fraction_range=(0.0, 0.0),
            reset_joint_noise_rad=0.0, reset_joint_velocity_rad_s=0.0,
            reset_linear_speed_m_s=0.0, reset_angular_speed_rad_s=0.0,
            reset_tilt_rad=0.0, reset_roll_phase_range_rad=(0.0, 0.0),
        )
    elif stage == "deploy_near_stand":
        result = replace(
            base,
            curriculum_stage=stage,
            reset_start_mode=int(TransitionMode3D.DEPLOY),
            reset_compact_fraction_range=(0.0, 0.20),
            reset_joint_noise_rad=0.02, reset_joint_velocity_rad_s=0.10,
        ...
        result = replace(
            ...
            reset_joint_noise_rad=0.04, reset_joint_velocity_rad_s=0.20,
...
def validate_transition_config_3d(config: Transition3DConfig) -> None:
    ...
    if not 0.0 < config.action_range_fraction <= 1.0:
        raise ValueError("action_range_fraction must be in (0, 1]")
    if not 0.0 < config.snapshot_tail_fraction <= 1.0:
